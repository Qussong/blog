...
[Collider2D.IsTouchingLayers](https://docs.unity3d.com/ScriptReference/Collider2D.IsTouchingLayers.html) : Collider2D 가 특정 레이어에 속한 다른 Collider 2D 와 접촉중인지 확인하는데 사용한다.</br>
```csharp
...
```
[LayerMask.GetMask](https://docs.unity3d.com/ScriptReference/LayerMask.GetMask.html) : 해당 함수는 레이어 이름을 받아서 해당 레이어들을 포함하는 비트마스크 값을 반환한다.</br>
```csharp
